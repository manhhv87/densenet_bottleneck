...
def get_challenge17_data(db_dir, fs=None, pad=None, normalize=False, verbose=False):
    ...
    if normalize:
        normalize = functools.partial(physionet.normalize_challenge17, inplace=True)

    data_set = _prepare_data(records, labels, normalize_fn=normalize,
                             fs=fs, pad=pad, verbose=verbose)
    return data_set
...
def get_challenge20_data(db_dir, fs=None, pad=None, normalize=False, verbose=False):
    ...
    if normalize:
        normalize = functools.partial(physionet.normalize_challenge20, inplace=True)
...
def _prepare_data(records, labels, normalize_fn=None, fs=None, pad=None, verbose=False):
    x = _transform_records(records, fs=fs, pad=pad,
                           normalize=normalize_fn, verbose=verbose)
    data_set = {'x': x,
                'y': labels.to_numpy(),
