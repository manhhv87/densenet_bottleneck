...
def _create_model(n_classes, act, dat_x, weights_file):
    ...
    if weights_file:  # pre-trained weigh
        ...
        old_model.load_weights(str(weights_file)).expect_partial()

    old_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3, beta_1=0.9,
    ...
    if not args.k_fold:  # Không sử dụng k-fold
        ...
        with strategy.scope():
            ...
            if args.val_metric in ['loss', 'acc']:
                ...
                checkpoint = [
                    tf.keras.callbacks.ModelCheckpoint(filepath=str(args.job_dir / 'backup_model_last.weights')),
            ...
            print('[INFO] Loading the best weights from file {} ...'.format(
                str(args.job_dir / 'backup_best_model.weights')))
        ...
        for train_idx, val_idx in kf.split(idx_data):
            ...
            val_data = _create_dataset_from_data(val).with_options(options).batch(args.batch_size, drop_remainder=True)
