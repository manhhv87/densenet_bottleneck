...
# %% Constants
score_fun = {'AUC': auc, 'Fmax': f_max}
diagnosis = ['SR','AFIB',	'STACH', 'SARRH', 'SBRAD', 'PACE', 'SVARR', 'BIGU', 'AFLT', 'SVTAC', 'PSVT', 'TRIGU']
nclasses = len(diagnosis)
predictor_names = ['DNN']

bootstrap_nsamples = 500
...
threshold = np.array([0.395, 0.339, 0.736, 0.355, 0.450, 0.149, 0.185, 0.841,
                      0.280, 0.853, 0.429, 0.315])  # corresponding to 'Normal', 'AF', 'I-AVB', 'LBBB', 'RBBB', 'PAC', 'PVC', 'STD', 'STE'

# %% Read datasets
# Get true values
y_true = pd.read_csv('./data/annotations/ptb_xl_gold_standard.csv').values

# get y_score for different models
y_score_list = [pd.read_csv('./data/dnn_predicts/other_seeds/model_' + str(i + 1) + '.csv').values for i in range(1)]
y_score_list = [y_score[:, 13:].astype(np.float64) for y_score in y_score_list]
