...
#                             between the context and the positive sample. If the offset is 0 then the positive sample
#                             comes directly after the context.
#     @param ns_same_segment_prob: Probability, that a negative sample will come from the same segment as
#                                  the positive sample. If the probability is 0 then all negative samples
#                                  are equally likely to come from every segment in the buffer. If the probability
#                                  is 1 then all negative samples come from the same segment as the positive sample.
#                                  By default, all segments have the same probability of being sampled.
...
#                            of the positive sample in the samples array. Context is an array of (optionally)
#                            overlapping frames. Samples is an array of frames to predict from (1 positive sample
#                            and rest negative samples).
#     """
...
#     for patients_buffer in buffered_patient_generator:  # patients_buffer = (id, data)
#         for _ in range(samples_per_patient):
...
#
#     @param patients: An array of tuples of patient's id and data.
#     @param size: Number of the returned random segments. Defaults to 1 returned random segment.
#     @param segment_p: Fixed probability of a chosen segment.
#                       `segment_p` should be a tuple of: (patient_index, segment_index, segment_probability)
#
#     @return: One or more tuples (patient_index, segment_index) describing
#              the randomly sampled segments from the patients buffer.
#     """
#     num_segments_per_patient = np.array([signal.shape[0] for _, (signal, _) in patients])   # patients = (id, data, label)
#                                                                                             # return no. of segments
